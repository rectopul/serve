...
function custom_comments($comment, $args, $depth) {
  ...
  ?>
   <li id="comment-<?php comment_ID() ?>" <?php comment_class() ?>>
    ...
    <div class="comment-autor-name"><?php echo get_comment_author_link(); ?></div>
    <div class="comment-meta"><?php printf(__('Posted %1$s at %2$s <span class="meta-sep">|</span> <a href="%3$s" title="Permalink to this comment">Permalink</a>', 'serve'),
       get_comment_date(),
       get_comment_time(),
       '#comment-' . get_comment_ID() );
       edit_comment_link(__('Edit', 'serve'), ' <span class="meta-sep">|</span> <span class="edit-link">', '</span>'); ?></div>
  ...
  <?php // echo the comment reply link
   if($args['type'] == 'all' || get_comment_type() == 'comment') :
    comment_reply_link(array_merge($args, array(
     'reply_text' => __('Reply','Serve'),
     'login_text' => __('Log in to reply.','serve'),
     'depth' => $depth,
...
function custom_pings($comment, $args, $depth) {
       $GLOBALS['comment'] = $comment;
        ?>
      <li id="comment-<?php comment_ID() ?>" <?php comment_class() ?>>
       <div class="comment-author"><?php printf(__('By %1$s on %2$s at %3$s', 'Serve'),
